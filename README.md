...
基于 Flask 的 AI 智能作业批改 Web 应用。用户上传作业图片，系统自动完成图像增强、OCR 识别、大模型批改，并生成可视化的批改结果图。

## 功能特性

- **用户认证**：注册、登录、退出登录、会话管理
- **文档扫描增强**：OpenCV 边缘检测 + 透视变换拉正 + 对比度增强与锐化
- **OCR 文字识别**：对接夸克 RecognizeQuestion 接口，支持多格式图片
- **AI 智能批改**：调用千问大模型（qwen-plus）批改作业，返回结构化批改结果
- **批改结果可视化**：将批改结果渲染为图片（题号、正误判断、标准答案、解题解析）
- **批改记录管理**：历史记录保存、查看详情、删除
